...
<div class="content box_m" role="main">
  ...
  <a class='bton1' name='username'>Check Student
  </a>
  </button>
	</td>
  </form>
  ...
  <table>
    <tr>
    <form action='student_for_teacher.php' method='POST'>
...
$teacher=$_SESSION['username'];
				...
				if(!mysqli_connect_error()){
					...
					$select_data="SELECT s.name,ss.username,s.year,sj.sub,s.sem,ss.t_status From stud_sub ss,student s,subject sj Where ss.year = ? and ss.username=s.username and sj.sub=ss.sub and sj.teacher=? order by ss.t_status";
					
					
					$stmt=$conn->prepare($select_data);
					$stmt->bind_param("ss",$year,$teacher);
					$stmt->execute();
            ...
            echo("<table><tr>");
              echo ("<td><h3>Student Name----</td>    <td><h3>Subject Name----</td>    <td><h3>Your Status----</td>   <td><h3>Make Done</td></tr>");
                ...
                $var=$row['username'];
                echo ("<tr><form action='done_by_teacher.php' method='POST'>");
                echo ("<td>".$row['name']."</td>   <td>".$row['sub']."</td>    <td>".$row['t_status']."</td>");
                echo("<td>");
