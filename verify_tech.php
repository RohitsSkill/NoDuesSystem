...
//$year=$_POST['year'];
				...
				if(!mysqli_connect_error()){
					...
					$select_data="SELECT name,username,lab_no From users where post!=? order by name";
					
					$dont_select="hod";
          $stmt=$conn->prepare($select_data);
          $stmt->bind_param('s',$dont_select);
					$stmt->execute();
            ...
            echo("<table><tr>");
              echo ("<td><h3>Teacher Name----</td>    <td><h3>Lab No----</td>    <td><h3>Accept</td></tr>");
                ...
                $i++; 
                $var=$user['username'];
                echo ("<tr><form action='verify_teacher.php' method='POST'>");
                echo ("<td>".$user['name']."</td>   <td>".$user['lab_no']."</td>");
                echo("<td>");
                echo("<button  name='username' value=$var>Accept</button>");
                echo("</form></td>");
                echo ("</tr>");
           //     echo $user['name'];
							}
